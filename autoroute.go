...
func AutoRoute(handler any) gin.HandlerFunc {
	return findAndCall(handler, true, parseMethodAndArgs)
}

func AutoRouteAny(handler any) gin.HandlerFunc {
...
func findHandlerFuncs(handler any, onlyCtx bool) map[string]reflect.Value {
...
func REST(handler any) gin.HandlerFunc {
	return findAndCall(handler, true, parseRESTMethodAndArgs)
}

func RESTAny(handler any) gin.HandlerFunc {
	return findAndCall(handler, false, parseRESTMethodAndArgs)
}

func findAndCall(handler any, onlyCtx bool, finder func(c *gin.Context) (string, []string)) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if !fn.Type().IsVariadic() && !onlyCtx && len(args) > numIn-1 {
			http.NotFound(c.Writer, c.Request)
			c.Abort()
			return
		}
